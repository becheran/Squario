...
void TitleScreen()
{
  ...
  while (true)
  {
    if (Game.ButtonOffCD())
    {
      ...
      if (input & A_BUTTON || input & B_BUTTON)
        break;
      if ((input & LEFT_BUTTON) || (input & RIGHT_BUTTON))
        SoundOn = !SoundOn;
      if (input & UP_BUTTON)
      {
        if (RandomSeedSeed == 255)
          RandomSeedSeed = 1;
        else
          RandomSeedSeed++;
      }
    ...
    display.print(F("Seed:  "));
    display.print(RandomSeedSeed);
  ...
  for (uint8_t a = 0; a < GameSeeds; a++)
    Game.Seeds[a] = random(255);
}
...
void SoundEngine()
{
  ...
  {
    if (SoundCounter < 8)
      display.tunes.playScore(Verse);
    else if (SoundCounter < 12)
      display.tunes.playScore(PreChorus);
    else if (SoundCounter < 16)
      display.tunes.playScore(Chorus);
    SoundCounter++;
    if (SoundCounter == 16)
      SoundCounter = 0;
  }
...
void displayHighScores(byte file)
{
  ...
  while (true)
  {
    if (display.buttonsState() && Game.ButtonOffCD())
      return;
  }
}
